<?php 
	...
	class Usuario{
		...
		public function atualizaPerfil($codUsuario, $nickName, $emailUsuario, $senha, $areaAtuacao){
			...
			$verifica = $sql->select("SELECT * FROM areaatuacao_usuario WHERE COD_USUARIO = :CODUSUARIO", array(
									 ":CODUSUARIO"=>$codUsuario
									 ));
			if(count($verifica) > 0){
				...
				$sql->query("INSERT INTO areaatuacao_usuario (COD_USUARIO, COD_AREA)
							 VALUES (:CODUSUARIO, :CODAREA)", array(
							":CODUSUARIO"=>$codUsuario,
							":CODAREA"=>$areaAtuacao
							));
			}
